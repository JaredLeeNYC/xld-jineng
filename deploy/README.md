...
- 恢复前必须进入维护窗口并停止 `skill-matrix-server`。恢复演练执行 `deploy/restore-materials.sh /var/backups/skill-matrix-materials.tar.gz /var/lib/skill-matrix/materials-restore-test`；脚本持有维护锁、拒绝活动上传，并在原子切换前后逐文件校验 SHA-256。
